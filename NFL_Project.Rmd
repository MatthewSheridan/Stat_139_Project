...
#Stepwise Models
#Stepwise Backward
step.back = step(baseline.lm, direction = "backward", k = 2, trace = F)
formula(step.back)
```
...
```{r}
#Stepwise Both
step.both = step(baseline.lm, scope = c(lower = formula(Score_Dif ~ 1), upper = formula(Score_Dif ~ (. - Game_id - Team_Name)^2)), direction = "both", trace = F)
formula(step.both)
```
