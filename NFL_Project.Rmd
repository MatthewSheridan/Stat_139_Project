...
lmer_step <- lmer(Score_Dif ~ Home + QB_Hit + Pass_Yrd + Rush_Yrd + FG_attempt + 
    ...
    RedZone_Plays + Unique_Receivers + Shotgun_Play + (1|Team_Name), 
...
#Stepwise Both
step.both = step(step.back, scope = c(lower = Score_Dif ~ . - Game_id - Team_Name, upper = formula(Score_Dif ~ (. - Game_id - Team_Name))), direction = "both", trace = F)
formula(step.both)
```
